          ...
          >
            <div class="card-body p-4 p-lg-5">
              <section class="mb-4">
                ...
                <div v-if="form.visibility === 0" class="mt-4">
...
async function handleSubmit() {
  ...
  if (form.visibility === 0 && !form.password.trim()) {
  ...
  const payload = {
    ...
    ...(form.visibility === 0 ? { password: form.password } : {}),
  ...
  try {
    ...
    submitError.value = resp.message || '竞赛创建失败，请稍后重试。'
  } catch (error) {
    submitError.value = error?.message || '竞赛创建失败，请稍后重试。'
  } finally {
